...
if(isset($_POST["simpan2"]))
{
    ...
    $totalhargasewa = $db->__construct()->escape_string($_POST["totalhargasewa"]);
    $totalhargasewa1 = filter_var($totalhargasewa, FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_THOUSAND);
    $metodebayar = $db->__construct()->escape_string($_POST["metodebayar"]);
    $idcustomer  = $_SESSION["id_customer"];

    $query = mysqli_query($db->__construct(), "SELECT id_karyawan as idkaryawan FROM karyawan ORDER BY RAND() LIMIT 5;");
    $data = mysqli_fetch_array($query);
    $idkaryawan = $data['idkaryawan'];

    $query1 = mysqli_query($db->__construct(), "SELECT max(id_destinasi) as iddestinasi from destinasi");
    $data1 = mysqli_fetch_array($query1);
    $iddestinasi = $data1['iddestinasi'];

    $query2 = mysqli_query($db->__construct(), "SELECT max(id_pembayaran) as idpembayaran from pembayaran");
